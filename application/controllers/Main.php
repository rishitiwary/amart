...
// 	        redirect(site_url().'main/login/');
// 	    }
// 	    else if(!empty($data['role']) && $data['role']!=1){
//             redirect(site_url().'main/login/');
//         }
// 	    else{
// 	        $dataLevel = $this->userlevel->checkLevel($data['role']);
        ...
        if(!empty($data['email'])){
            ...
            if($this->form_validation->run() == FALSE) {
                ...
                if($data['recaptcha'] == 'yes'){
                    ...
                    if(!$userInfo)
                    {
                        $this->session->set_flashdata('flash_message', 'Your Credentials are invalid please check with Admin!');
                        redirect(site_url().'main/login');
                    }
                    elseif($userInfo->banned_users == "ban")
                    {

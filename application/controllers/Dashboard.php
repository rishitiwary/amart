 ...
 public function editaddress($address_id)
    {
        ...
        $this->load->view('home/header', $data);
        $data['edit_data']=$this->basic_operation->uniqueSelect('address_books',array('id'=>$address_id));
        $this->load->view("home/edit_address", $data);
        $this->load->view('home/footer'); 
    }
    ...
    public function updateAddress($address_id)
    {
        ...
        if ($this->form_validation->run() == TRUE) 
        {
            ...
            $hidden_address_id=$this->input->post('hidden_address');
            //$msg=$this->Commonmodel->insert("address_books", $mydata);
            $update=$this->Commonmodel->eidt_single_row('address_books', $mydata,"id=$hidden_address_id");          
            $this->session->set_flashdata('success', 'Address Updated Successfully!');        
            redirect('dashboard/address','refresh');
         }else {
    ...
    {
        $result = $this->Commonmodel->delete_single_con('address_books', "id='$address_id'");
        if($result)
        {
        ...
        if($this->form_validation->run() == TRUE)
        {
          ...
          $coupon_id=$this->input->post('coupon_id')?$this->input->post('coupon_id'):0;
            ...
            //   $message .= 'Ashamart Team';
              ...
              $this->db->where(array('bookings.user_id'=>$user_id,'bookings.order_id'=>$order_id,'bookings.product_type'=>'1'));
              ...
              $this->db->join('coupons', 'coupons.coupon_id = bookings.coupon_id','left');
              $this->db->join('address_books', 'address_books.id = bookings.address_id');
              $this->db->where(array('bookings.user_id'=>$user_id,'bookings.order_id'=>$order_id,'bookings.product_type'=>'2'));
              ...
              $this->db->join('users as customer', 'customer.id = bookings.user_id');
              $this->db->join('address_books', 'address_books.id = bookings.address_id');
              $this->db->where(array('bookings.user_id'=>$user_id,'bookings.order_id'=>$order_id,'bookings.product_type'=>'3'));
              $data3= $this->db->get()->result();
              ...
              $fetch_data= $orderData['fetch_data'];
              ...
              $message='';
            $message .='<html><head><meta charset="utf-8"><title>Ashamart </title></head>';
            $message .='<body style="margin: 0; padding: 0;">';
            $message .='<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  style="background:#efefef"><tr><td>';
            $message .='<table width="800" border="0" align="center" cellpadding="0" cellspacing="0" style="font-family: "Poppins", sans-serif; background: #fff">';
            $message .='<tbody><tr><td>';
            $message .='<table width="100%" align="center">';
            $message .='<tr><td align="center" style="border-bottom: 8px solid green; padding: 10px 0"><img src="'.base_url().'images/common/logo.png" height="100" alt="Ashamart"/></td></tr>';
            $message .='</table>';
            $message .='<table width="100%" align="center" style="background: #F5F5F5">';
            $message .='<tr><td height="100" align="center" valign="bottom"><img src="'.base_url().'images/common/tick.png" width="59" height="59" alt="Ashamart"/></td></tr>';
            $message .='<tr><td height="70" align="center" valign="top" style="font-size: 20px; font-weight: bold">Thank You for your order at Ashamart</td> </tr></table>';
            $message .='<table width="100%" align="center" style="background: #E3E3E3; padding: 30px 30px; font-size: 14px">';
            $message .='<tr><td width="50%" align="left" valign="top"><table><tr><td><strong>Order No -</strong></td></tr><tr><td>'.$fetch_data[0]->order_id.'</td></tr><tr><td height="10"></td></tr>';
            $message .='<tr><td><strong>Booking Slot</strong></td></tr><tr><td>'.date("F j, Y",strtotime($fetch_data[0]->booking_date)) .'</td></tr><td></td></table></td>';
            $message .='<td width="50%" align="left" valign="top"><table><tr><td><strong>Delivery Slot</strong></td></tr><tr><td>'. date("F j, Y",strtotime($fetch_data[0]->delivery_date)) .'</td></tr>';
            $message .='<tr><td><strong>Delivery Address</strong></td></tr><tr><td height="10"></td></tr><tr><td><strong>'.$fetch_data[0]->name.'</strong></td></tr><tr><td>'.$fetch_data[0]->address.'</td></tr>';
            $message .='<tr><td><strong>Mobile - '.$fetch_data[0]->mobile.'</strong></td></tr></table></td></tr></table>';
            $message .='  <table width="100%" cellpadding="0" cellspacing="0" style="background: #fff;  font-size: 14px"><tr style="background: #F5F5F5; font-weight: bold"><td style="padding-left:30px" height="40" valign="middle">Sr. No</td>';
            $message .=' <td style="padding-left:30px" height="40" valign="middle">Item Details</td><td style="padding-left:30px" height="40" valign="middle">Qty</td><td style="padding-left:30px" height="40" valign="middle">Unite Price</td>';
            ...
            { 
                ...
                $message .='<tr><td style="padding-left:30px; border-bottom: 1px solid #F5F5F5 " height="40">'. $counter.'</td>';
                $message .='<td style="padding-left:30px; border-bottom: 1px solid #F5F5F5" height="40">['.$row->product_code.'] '.$row->product_name.'</td>';
                $message .='<td style="padding-left:30px; border-bottom: 1px solid #F5F5F5" height="40">'.$row->cart_product_quantity.'</td>';
                $message .='<td style="padding-left:30px; border-bottom: 1px solid #F5F5F5" height="40">AED'. number_format(@$row->offer_price, 2).'</td>';
                 $message .='<td style="padding-left:30px; border-bottom: 1px solid #F5F5F5" height="40">₹ '.number_format(($row->cart_product_quantity*$row->offer_price),2).'</td>';
               $message .=' </tr>';
                $counter++;
            } 
            $deliveryCharge=$subTotal > 80 ? 0 : 10;
            ...
            $message .='<tr><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Sub Total</strong></td>';
            $message .='<td style="padding-left:30px" height="40"><strong>AED'.' '.number_format($subTotal,2).'</strong></td></tr>';
            $message .='<tr><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>VAT 5%</strong></td>';
            $message .='<td style="padding-left:30px" height="40"><strong>₹ ';
            
            $vat= (5 / 100) * @$subTotal ; 
            $message .= number_format($vat, 2);
            $message .='</strong></td></tr><tr><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Delivery Charges</strong></td>';
            $message .='<td style="padding-left:30px" height="40"><strong> ₹ '.' '.number_format($deliveryCharge,2).'</strong></td></tr> ';
            $sameDay= 0;
            ...
            if($type=="Same day delivery")
            {
             ...
             $message .='<tr><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Same Day Delivery Charges</strong></td><td style="padding-left:30px" height="40"><strong>₹ '.number_format($sameDay,2).'</strong></td></tr>';
            $deliveryCharge = $deliveryCharge +$vat +$sameDay;
             if($fetch_data[0]->coupon_id <= 0){
                $message .='<tr style="background: #383838 ; color: #fff"><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Grand Total</strong></td>';
                $message .='<td style="padding-left:30px" height="40"><strong>';
                
                $grandtotal= $deliveryCharge+$subTotal ; 
                $message .= '₹ '.number_format($grandtotal,2);
                $message .='</strong></td></tr>';
                    ...
                    { 
                       ...
                       $message .='<tr style="background: #383838 ; color: #fff"><td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Grand Total</strong></td>';
                       $message .='<td style="padding-left:30px" height="40"><strong>';
                                 $grandtotal=$deliveryCharge+abs($amount); 
                                echo  '₹ '.number_format($grandtotal, 2);
                        $message .='</strong></td></tr> ';
                     } else { 
                            $amount=$fetch_data[0]->discount_percentage/100*$subTotal;
                            $amount=$subTotal-$amount;
                            $message .='<tr style="background: #383838 ; color: #fff">';
                            $message .='   <td height="40" colspan="4" align="right" style="padding-right:30px; border-bottom: 1px solid #F5F5F5"><strong>Grand Total with ';
                            $message .=  $fetch_data[0]->discount_percentage.' % Discount(Coupon Applied)'.':</strong></td>';
                            $message .= '<td style="padding-left:30px" height="40"><strong>';
                ...
                $message .= '</table><table width="100%"><tr align="center"><td height="50" align="center" valign="bottom">Happy Shopping</td></tr>';
                $message .= ' <tr><td height="50" align="center" valign="top" style="font-size: 20px; font-weight: bold">Team Ashamart</td></tr>';
                $message .= '</table><table width="100%" style="background: #F5F5F5; padding: 20px 0 "><tr align="center"><td height="50" colspan="2" align="center" style="font-size: 20px; font-weight: bold" valign="middle">DOWNLOAD OUR APP</td></tr>';
                $message .= '<tr><td align="right" valign="middle" style="padding-right: 5px"><a href="#"><img src="'.base_url().'images/common/google.png" width="140" height="40" alt=""/></a></td>';
                $message .= '<td align="left" valign="middle" style="padding-left: 5px"><a href="#"><img src="'.base_url().'images/common/ios.png" width="140" height="40" alt=""/></a></td></tr>';
                $message .= '</table><table width="100%" style="background:#B60008; color: #fff; padding: 30px"><tr><td><strong>Have questions?</strong></td></tr><tr><td>We are here to help, learn more about us here  <a href="#" style="color: #fff; text-decoration: none">Contact us</a></td></tr>';
                $message .= '</table></td></tr></tbody></table></td></tr></table></body></html>';
               // echo  $message ;exit;
               $to_email = $email;
               ...
               if($this->email->send())
               {
                ...
                redirect('dashboard/thankyou','refresh');

 ...
 { 
    ...
    public function index22()
    {
     ...
     {   
       ...
       $banners = $this->basic_operation->Uniqueselect('galleries',array('status'=>1));
       ...
       $new_arraival =  $this->Commonmodel->fectch_all_by_new();
    //   print_r($new_arraival);
    //   exit();
     ...
     {  
      ...
      $data['bestsellinglist']= $this->Commonmodel->fetch_all_rows_limit('promotional', "category_id=1 and status=1", 4);

      $data['offerslist']=$this->Commonmodel->fetch_all_rows_limit('offers', "status=1", 4);

     if($type=='3')
      {
        $sql = "SELECT promotional.* from promotional JOIN promotional_category ON promotional.category_id = promotional_category.id WHERE promotional.category_id=$cat and promotional.status=1";
        ...
        $sql = "SELECT offers.* from offers WHERE offers.status=1 and offers.category_id in (SELECT id from categories WHERE (categories.id=$cat OR categories.parent_category=$cat) AND categories.parent_category!=0)";
        ...
        $sql ="SELECT products.* FROM products WHERE products.status=1 and products.category_id in (SELECT id from categories WHERE (categories.id=$cat OR categories.parent_category=$cat) AND categories.parent_category!=0)";
        $data['cat']= $this->Commonmodel->fetch_row('categories', "categories.id=$cat");
        $where= array('categories.status'=>1 , 'categories.parent_category'=>$cat);
        $data['subcatlist']= $this->Commonmodel->fetch_all_rows('categories', $where);
      }  
    ...
    {         
      ...
      if($type=='3'){
        $sql = "SELECT promotional.* from promotional JOIN promotional_category ON promotional.category_id = promotional_category.id WHERE promotional.status=1 and promotional.category_id=$subcat";
        ...
        $sql = "SELECT offers.* from offers JOIN categories ON offers.category_id = categories.id WHERE offers.status=1 and category_id=$subcat GROUP BY products.id";
        $data['cat']= $this->Commonmodel->fetch_row('categories', "categories.id=$subcat");
        $where= array('categories.status'=>1 , 'categories.parent_category'=>$cat);
        $data['subcatlist']= $this->Commonmodel->fetch_all_rows('categories', $where);
      }else{
        $sql = "SELECT products.* from products JOIN categories ON products.category_id=categories.id WHERE products.status=1 and products.category_id=$subcat";
        $data['cat']= $this->Commonmodel->fetch_row('categories', "categories.id=$subcat");
        $where= array('categories.status'=>1 , 'categories.parent_category'=>$cat);
        $data['subcatlist']= $this->Commonmodel->fetch_all_rows('categories', $where);
      }
  ...
  {        
      ...
      $q = "SELECT products.product_code, products.name, products.offer_price, products.price, products.offer_percentage, products.country, products.category_id, products.product_type, products.description,products.id FROM `products` INNER JOIN `categories` ON `categories`.`id`=`products`.`category_id` AND products.name LIKE '%$keyword%'
      where products.status=1
      UNION
      SELECT offers.product_code, offers.name, offers.offer_price, offers.price, offers.offer_percentage, offers.country, offers.category_id, offers.product_type, offers.description,offers.id FROM `offers` INNER JOIN `categories` ON `categories`.`id`=`offers`.`category_id` AND offers.name LIKE '%$keyword%'
      where offers.status=1
      UNION
      SELECT promotional.product_code,promotional. name,promotional.offer_price, promotional.price, promotional.offer_percentage, promotional.country, promotional.category_id, promotional.product_type, promotional.description,promotional.id FROM `promotional` INNER JOIN `promotional_category` ON `promotional_category`.`id`=`promotional`.`category_id`
      AND promotional.name LIKE '%$keyword%' where promotional.status=1
      ";
